(function($) {
    ...
    HT.menuRowHtml = (option) => {
        ...
        const isLevel1 = window.location.href.indexOf('/children/') === -1;
        
        let colums;
        if (isLevel1) {
            colums = [
                { class: 'col-lg-3', name: 'menu[name][]', value: (typeof(option) != 'undefined') ? option.name : '' },
                { class: 'col-lg-3', name: 'menu[canonical][]', value: (typeof(option) != 'undefined') ? option.canonical : '' },
                { class: 'col-lg-3', name: 'menu[icon][]', value: (typeof(option) != 'undefined' && option.icon) ? option.icon : '', inputClass: 'upload-image', readonly: true, placeholder: 'Chọn Icon (Menu cấp 1)' },
        ...
        colums.forEach(col => {
            ...
            .addClass('form-control '+ ((col.name == 'menu[order][]') ? 'int text-right' : '') + ' ' + (col.inputClass || ''))
            .attr('name', col.name)
            if (col.readonly) {
